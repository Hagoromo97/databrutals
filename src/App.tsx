  ...
  return (
    <div className="flex flex-1 flex-col gap-6 p-4 md:p-8 max-w-3xl mx-auto w-full overflow-y-auto" style={{ paddingBottom: 'calc(2rem + env(safe-area-inset-bottom))' }}>
      ...
      <div className="rounded-2xl border border-border overflow-hidden shadow-sm">
        {/* Collapsible header */}
        <button
          className="w-full grid grid-cols-4 bg-muted/50 text-xs font-semibold uppercase tracking-wide text-muted-foreground px-4 py-3 gap-2 hover:bg-muted/60 transition-colors duration-200 text-left items-center"
        ...
        {tableOpen && DAYS.map((day, i) => {
          ...
          <div
            key={day.en}
            className={`grid grid-cols-4 items-center px-4 py-3 gap-2 border-t border-border/60 transition-colors duration-200 ${isToday ? "bg-muted/50 dark:bg-muted/30" : "hover:bg-muted/20"}`}
          >
            <div className="flex items-center gap-2">
              {isToday && <span className="inline-block w-1.5 h-1.5 rounded-full bg-primary shrink-0" />}
              <div>
                <p className={`text-sm font-semibold ${isToday ? "text-foreground" : "text-foreground/80"}`}>{day.en}</p>
              </div>
            </div>
        ...
        {tableOpen && (
          <button
            className="w-full flex items-center justify-center gap-1 py-2 text-xs text-muted-foreground hover:text-foreground/80 hover:bg-muted/20 transition-colors duration-200 border-t border-border/60"
      ...
      <div className="rounded-2xl border border-border overflow-hidden shadow-sm">
        <button
          className="w-full flex items-center gap-1.5 bg-muted/50 text-xs font-semibold uppercase tracking-wide text-muted-foreground px-4 py-3 hover:bg-muted/60 transition-colors duration-200 text-left"
